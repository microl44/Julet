...
if(isset($_POST['pickedGenre']) && (isset($_SESSION['username']) || isset($_SESSION['password'])) ){
    ...
    $conn=GetConnection();
    $stmt = $conn->prepare('UPDATE genre SET next = 0;');
    $stmt->execute();
    $stmt->closeCursor();

    $stmt = $conn->prepare('UPDATE genre SET next = 1 WHERE name = :genre');
...
if(isset($_SESSION['username']) || isset($_SESSION['password'])){
    ...
    $conn = GetConnection();
    $stmt = $conn->prepare("SELECT name FROM genre WHERE next = 1;");
    ...
    $stmt->closeCursor();
    $conn = GetConnection();
    $stmt = $conn->prepare("SELECT name FROM genre WHERE next = 0;");
